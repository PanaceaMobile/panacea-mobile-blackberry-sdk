...
import com.panaceamobile.panacea.sdk.web.ApplicationUtils;
import com.panaceamobile.panacea.sdk.web.ConnectionManager;
import com.panaceamobile.panacea.sdk.web.InputSimUtils;
...
public class ContextImpl implements Context {
	...
	//		Log.e(TAG, "error in makePath() opening path "+path, ioe);
		}
		...
		//		Log.e(TAG, "error making last part of path " + path, e);
			}
		}
	...
	//		Log.d(TAG, "Error checking if file exists");
		} finally {
			if (fconn != null) {
				try {
					fconn.close();
	...
	private FileConnection open(String filename, boolean overwrite, boolean create) {
		...
		try {
			...
			if (!fconn.exists()) {
				if (create) {
	     //  	 		Log.d(TAG, "File not found, creating...");
					fconn.create();
				} else {
					// doesn't exist and told not to create, so return null
	   //    	 		Log.d(TAG, "File not found, returning null...");
					try {
						fconn.close();
					} catch (IOException ioe) {}
					return null;
	 ...
	 //      	 	Log.d(TAG, "Existing file found, recreating...");
				// recreate file
	       	 	fconn.truncate(0);
			}
			return fconn;
	...
	//		Log.e(TAG, "Error confirguring email.", e);
			return;
		}		
	...
	//		Log.e("ContextImpl.fileList()", "Failure to list files.");
		} finally {
			try {
				rootConn.close();						
			} catch (Exception e) {				
